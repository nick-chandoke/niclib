...
randDist :: Monoid m
         => (a -> m) -- ^ lift an object into a monoid, e.g. 'Set.singleton' or @(:[])@
         -> Int -- ^ number of elements to generate
         -> RVar a -- ^ e.g. 'Data.Random.Distribution.Uniform.integralUniform'
         -> IO m
randDist l i r = replicateM' i $ l <$> runRVar r DevURandom -- DevURandom generates pseudorandom numbers when the entropy pool is dry
