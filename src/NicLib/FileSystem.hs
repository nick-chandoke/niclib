...
import System.Posix.Files (createSymbolicLink, readSymbolicLink, removeLink)
...
lsRel :: (MonadIO m, MonadCatch m, LL.ListLike s Char, IsString s) => String -> ExceptT s m [String]
lsRel fp@(FilePath -> fp') = do
    liftIO (D.doesDirectoryExist fp) >>= flip boolToErr (fromString $ "The directory " ++ fp ++ " does not exist (perhaps something by that name exists, but isn't a directory?)") . not
...
lsRel' :: (MonadIO m, MonadCatch m, LL.ListLike s Char, IsString s, Monoid s, Eq s) => String -> WriterT [s] m [String]
...
subDirsS :: (LL.ListLike s Char, MonadIO m, MonadCatch m, IsString s, Eq s, Monoid s) => (String -> m Bool) -> String -> WriterT [s] m (S.Set String)
...
subDirsT :: (LL.ListLike s Char, IsString s, Eq s, MonadIO m, MonadCatch m) => (String -> m Bool) -> (String -> a) -> String -> WriterT [s] m (Tree a)
...
subDirsIO :: (MonadIO m, MonadCatch m, LL.ListLike s Char, IsString s, Eq s) => (String -> m Bool) -> (String -> m ()) -> String -> WriterT [s] m ()
subDirsIO p m fp = do
...
mv, cp, cpWithMetadata, cpPermissions :: (MonadIO m, MonadCatch m, LL.ListLike s Char, IsString s) => String -> String -> ExceptT s m ()
...
mkdir :: (MonadIO m, MonadCatch m, ListLike s Char, IsString s) => String -> ExceptT s m ()
...
mkcd :: (MonadIO m, MonadCatch m, ListLike s Char, IsString s) => String -> ExceptT s m ()
mkcd = mkdir &&& cd >*> (>>)
...
ln :: (MonadIO m, MonadCatch m, LL.ListLike s Char, IsString s) => String -> Maybe String -> ExceptT s m ()
...
dirdiff :: (ListLike s Char, IsString s, Eq s) => String -> String -> WriterT [s] IO (S.Set String, S.Set String, S.Set String)
dirdiff a b = do
...
fileEq :: (MonadIO m, MonadCatch m, LL.ListLike s Char, IsString s) => String -> String -> ExceptT s m Bool
